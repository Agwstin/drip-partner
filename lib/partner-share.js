...
/** Campos booleanos por síntoma que sí se comparten (sin `note`) */
const PAIN_FIELDS = ['cramps', 'ovulationPain', 'headache', 'backache', 'nausea', 'tenderBreasts', 'migraine']
const MOOD_FIELDS = ['happy', 'sad', 'stressed', 'balanced', 'fine', 'anxious', 'energetic', 'fatigue', 'angry']

/**
 * Filtra un CycleDay a solo los campos compartibles.
 * Soporta la forma real de Realm (booleans por síntoma) y la legacy {value}.
 */
function sanitizeDay(day, includeBleeding) {
  ...
  if (day.pain) {
    out.pain = {}
    for (const f of PAIN_FIELDS) if (day.pain[f] === true) out.pain[f] = true
    if (typeof day.pain.value === 'number') out.pain = day.pain.value
    if (Object.keys(out.pain).length === 0) delete out.pain
  }
  if (day.mood) {
    out.mood = {}
    for (const f of MOOD_FIELDS) if (day.mood[f] === true) out.mood[f] = true
    if (typeof day.mood.value === 'number') out.mood = day.mood.value
    if (Object.keys(out.mood).length === 0) delete out.mood
  }
