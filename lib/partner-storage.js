...
export const partnerRelayObservable = Observable()
...
export function initPartnerStorage() {
  ...
  setObvWithInitValue('partnerRelay', partnerRelayObservable, '')
}

export async function savePartnerRelay(url) {
  await AsyncStorage.setItem('partnerRelay', url)
  partnerRelayObservable.set(url)
}
