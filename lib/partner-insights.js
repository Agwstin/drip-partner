 ...
 *
 * NOTA: los CycleDay reales de drip (Realm) tienen forma:
 *   pain:  { cramps: bool, headache: bool, ... }   (booleans por síntoma)
 *   mood:  { happy: bool, sad: bool, fatigue: bool, ... } (booleans)
 *   desire: { value: int }
 *   bleeding: { value: int }
 * Este módulo normaliza AMBAS formas (booleans y {value}) a series
 * numéricas comparables.
 *
 * @param {Array} cycleDays Días con síntomas, ordenados por fecha (ISO yyyy-MM-dd).
    ...
    const days = cycleDays
      ...
      .map((d) => {
        ...
        return {
          ...
          mood: normalizeMood(raw && raw.mood),
          pain: normalizePain(raw && raw.pain),
...
/** Pain real de drip: booleans por síntoma -> score 0..n (n = síntomas activos) */
export function normalizePain(pain) {
  if (!pain) return undefined
  if (typeof pain.value === 'number') return pain.value // forma legacy {value}
  const symptoms = ['cramps', 'ovulationPain', 'headache', 'backache', 'nausea', 'tenderBreasts', 'migraine']
  const active = symptoms.filter((s) => pain[s] === true).length
  return active > 0 ? active : undefined
}

/** Mood real de drip: booleans -> escala -2..+2 (negativo..positivo) */
export function normalizeMood(mood) {
  if (!mood) return undefined
  if (typeof mood.value === 'number') return mood.value // forma legacy {value}
  let score = 0
  if (mood.happy) score += 1
  if (mood.balanced) score += 1
  if (mood.fine) score += 1
  if (mood.energetic) score += 1
  if (mood.sad) score -= 1
  if (mood.stressed) score -= 1
  if (mood.anxious) score -= 1
  if (mood.angry) score -= 1
  if (mood.fatigue) score -= 1
  return score
}
...
export function generateInsights(profile, correlations, worstDays) {
  ...
  // 1. Días de dolor recurrente (pain = conteo de síntomas activos)
  for (const w of worstDays) {
    if (w.avg >= 1) {
      const symptomLabel = w.avg >= 2 ? `${w.avg.toFixed(1)} síntomas de dolor` : 'dolor'
      insights.push({
        type: 'painPattern',
        phase: w.phase,
        text:
          `En los últimos ciclos, el día ${w.dayOfCycle} (${phaseName[w.phase]}) ` +
          `tu pareja suele registrar ${symptomLabel} (en ${w.n} ciclos). ` +
  ...
  // 3. Fase actual de mayor sensibilidad (mood: -2..+2, negativo = malestar)
    ...
    if (stats.mood.avg < 0) {
      insights.push({
        ...
        text:
          ...
          `(balance ${stats.mood.avg.toFixed(1)} en escala -2..+2, ${stats.mood.n} registros). ` +
