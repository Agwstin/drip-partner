    ...
    currEntries.forEach((entry) => {
      switch (entry.type) {
        case 'period':
          ...
          break
        case 'spotting':
          // si el mismo día ya hay period (sangrado real), el period gana
          if (dripEntry.bleedingValue === undefined) {
        ...
        case 'partying':
        case 'sleep_quality':
          ;(Array.isArray(entry.value) ? entry.value : []).forEach((item) => {
            dripEntry.noteValue = appendNote(dripEntry.noteValue, `[${entry.type}: ${item.option}]`)
          })
          break
        case 'sleep_duration':
          // Clue: value = { minutes: N } (objeto, no array)
          if (entry.value && typeof entry.value === 'object' && !Array.isArray(entry.value)) {
            const mins = entry.value.minutes
            if (mins) {
              const hours = Math.floor(mins / 60)
              const rem = mins % 60
              dripEntry.noteValue = appendNote(dripEntry.noteValue, `[sleep: ${hours}h${rem ? rem + 'm' : ''}]`)
            }
          }
          break
        default:
